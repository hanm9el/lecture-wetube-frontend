...
function SearchResults() {
    ...
    if (loading)
        return (
            <div className={twMerge(["p-10", "flex", "justify-center", "items-center"])}>
    ...
    if (videos.length === 0) {
        return (
            <div className={twMerge(["flex", "flex-col", "items-center", "justify-center", "h-[50dvh]"])}>
                <MdSubscriptions className={twMerge(["w-10", "h-10", "text-text-disabled"])} />
                <h2 className={twMerge(["text-xl", "font-bold", "mb-2"])}>새로운 동영상을 놓치지 마세요.</h2>
                <p className={twMerge(["text-text-disabled", "mb-6", "max-w-70", "text-center"])}>
    ...
    return (
        <div className={twMerge(["p-4", "sm:p-6"])}>
            <h1
                className={twMerge(["text-2xl", "font-bold", "mb-6"], ["flex", "items-center", "gap-3"])}
            >
                <MdVideoLibrary className={twMerge(["text-primary-main"])} />구독
            </h1>
