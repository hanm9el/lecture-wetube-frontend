...
import { createNotice } from "../../api/notice.ts";

type NoticeCreateFormData = {
    title: string;
    content: string;
};
...
function NoticeCreate() {
    ...
    const onSubmit = async (formData: NoticeCreateFormData) => {
        try {
            const result = await createNotice(formData.title, formData.content);
            // navigate("notices");
    ...
    return (
        <div className={twMerge(["max-w-4xl", "mx-auto", "px-4"])}>
            <h1 className={twMerge(["text-2xl", "font-bold", "mb-6"])}>공지사항 작성</h1>
            <form
            ...
            >
                ...
                >
                    <Button type={"button"} onClick={() => navigate("/notices")} variant={"ghost"}>
                        취소
                    </Button>
