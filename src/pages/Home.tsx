...
function Home() {
    ...
    const loadVideos = useCallback(async (pageNum: number) => {
        try {
            ...
            // 새로 도착한 result.videos에 filter로 걸러내기를 할건데
            // 그 새로운 요소(newV)를 순회하며 판단을 함
            // 기존 데이터(preV)에 조건(prevV의 id와 newV의 id가 같으면) 탈락되도록
            setVideos((prev) => {
                if (pageNum === 1) return result.videos;

                // 그냥 이대로 덧붙혀주기만 하면 시간차가 존재하기 때문에 중복이 될 수 있음
    ...
    // 초기 로딩에 대해서만 관여함
    // 왜냐하면, loadVideos는 useCallback으로 감싸줬기 때문에 변하지 않을 것임
    useEffect(() => {
        loadVideos(1).then(() => {});
    }, [loadVideos]);
    ...
    if (loading) {
        return <div className={twMerge(["p-8", "text-center", "text-text-disabled"])}>로딩 중 ...</div>;
    }
    ...
    return (
        <div className={twMerge(["p-4", "sm:p-6"])}>
            ...
            <div
                ref={observerTarget}
                className={twMerge(["h-10", "flex", "justify-center", "items-center", "w-full", "mt-8"])}
            >
