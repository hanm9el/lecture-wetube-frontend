...
function VideoCard({ video }: VideoCardProps) {
    return (
        ...
        >
            {/*썸네일*/}
            <div className={twMerge(["relative", "rounded-xl", "overflow-hidden"], ["aspect-video"])}>
                <img
            ...
            <div className={twMerge(["flex", "gap-3"])}>
                <Avatar nickname={video.author.nickname} src={video.author.profileImage} size={"sm"}></Avatar>
                <div className={twMerge(["flex", "flex-col"])}>
                    ...
                    <div className={twMerge(["text-text-disabled", "text-sm"])}>
                        ...
                        <p>
                            {/*
                            ~초 전, ~분 전, ~시간 전, ~일 전 같은 기능을 직접 구현할 수도 있음.
                            현재시각에서 업로드된 시간을 뺀 후,
                            60초 전이라면 ~초 전
                            60분 전이라면 ~분 전
                            24시간 전이라면 ~시간 전
                            ...
                            Date 객체는 다루기 어려움. 따라서 날짜/시간에 관련된 라이브러리가 많이 존재.
                            Moment.js, Day.js, data-fns 등이 많이 사용됨

                            dayjs(원하는 날짜)  ex. dayjs("2025-10-05")    -> Dayjs객체를 String으로 반환
                            ...
                            ~분 전, ~초 전 같은 현재 시간 대비 시간을 출력해주려면 을 컴포넌트 밖에서 써줘야 함.
