...
function CommentList({ videoId }: CommentListProps) {
    ...
    const onSubmit = async (e: FormEvent<HTMLFormElement>) => {
        ...
        try {
            const newComment: Comment = await createComment(videoId, content);
            // 등록이 끝났으니까 그 목록을 다시 로딩해서 화면에 출력해주는 것이 아니라
            // 우리가 갖고있는 목록에다가만 추가해주면 될 것임
    ...
    return (
        <div className={"mt-6"}>
            ...
            {/*댓글 입력 폼*/}
            {user && (
                <form onSubmit={onSubmit} className={twMerge(["flex", "flex-col", "gap-2", "mb-8"])}>
                    ...
                    <div className={twMerge(["flxe", "justify-end"])}>
                        <button
                            type={"submit"}
            ...
            <div className={twMerge(["space-y-6"])}>
                {comments.map((comment) => (
                    <div key={comment.id} className={twMerge(["flex", "justify-between", "gap-4", "group"])}>
                        <Avatar
                        ...
                        <div className={"flex-1"}>
                            ...
                            <p className={twMerge(["text-sm"])}>{comment.content}</p>
                        </div>
